...
import {selectAccount, twitchAccounts} from "./twitch-account";
...
function twitchSend() {
  ...
  if (data) {
    ...
    if(workStatus) {
      ...
      let currentIndex = ([...twitchAccounts].indexOf(currentAccount) + 1) % twitchAccounts.length;
      selectAccount(twitchAccounts[currentIndex].id);
